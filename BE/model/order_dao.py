...
class OrderDao:
    ...
    def get_order_logs(self, connection, order_filter):
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            SELECT 
                ...
                created_at
            FROM order_logs
            INNER JOIN order_statuses ON order_logs.id = order_statuses.id
            WHERE detailed_order_number = %(detailed_order_number)s 
    ...
    def update_delivery_info(self, connection, order_filter, order_update):
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            query = """
            ...
            """
            cursor.execute(query, order_filter, order_update)
            return 업뎃성공

    def update_order_status(self, connection, order_filter, order_update):
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            query = """
            UPDATE order_items
            SET 
            ...
            """
            cursor.execute(query, order_item_id, order_update)
